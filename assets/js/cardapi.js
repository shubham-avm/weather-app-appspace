/*! CardAPI v1.0.11-c916cbb | (c) Appspace | https://docs.appspace.com/faq/legal/ */
function CardApi(type) {
    ...
    this.init = function () {
        ...
        window.parent.postMessage({ message: "onapiready", cardId: cardId }, "*");
    }
    ...
    this.api = {
        xhr: function (url, settings) {
            ...
            window.parent.postMessage({ message: "api.xhr", $requestId: deferRunningId, url: url, settings: settings }, "*");
            //Return promise to api caller
            return defer.promise();
        },

        updateModelInput: function (input) {
            window.parent.postMessage({ message: "api.updatemodelinput", input: input }, "*");
        }
    }
    ...
    this.notifyOnLoad = function (p) {
        ...
        window.parent.postMessage({ message: "loaded", cardId: cardId }, "*");
    };
    ...
    this.notifyOnComplete = function (p) {
        ...
        window.parent.postMessage({ message: "complete", cardId: cardId }, "*");
    };
    ...
    this.notifyOnError = function (p) {
        ...
        window.parent.postMessage({ message: "error", cardId: cardId }, "*");
    };
